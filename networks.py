...
class SiameseNet(nn.Module):
    ...
    def forward(self, input):
        if self.arch == 'resnet':
            ...
            output = self.fc2(torch.abs(encode1 - encode2))
            # output = self.fc2(torch.cat((encode1, encode2), dim=1))
        elif self.arch == 'cnn':
            encode1 = self.fc5(self.conv4(input[:, 0]))
            encode2 = self.fc5(self.conv4(input[:, 1]))
            output = self.fc6(torch.abs(encode1 - encode2))
            # output = self.fc6(torch.cat((encode1, encode2), dim=1))

        return output


class ContrastiveNet(SiameseNet):
    def __init__(self):
        super().__init__()

    def forward(self, input):
        if self.arch == 'resnet':
            encode1 = self.fc2(self.conv1(input[:, 0, :, :]))
            encode2 = self.fc2(self.conv1(input[:, 1, :, :]))
        elif self.arch == 'cnn':
            encode1 = self.fc5(self.conv4(input[:, 0, :, :]))
            encode2 = self.fc5(self.conv4(input[:, 1, :, :]))
