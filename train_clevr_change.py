...
config = {
    ...
    'b_size': 256,
    'initial_lr': 0.0001,
    'beta1': [1,10], # style kl param
    'beta2': [10,1], # content kl param
...
b1_b2_records = []
for nd in numbered_dirs:
    config_path = os.path.join('experiments', config['experiment_setting'], str(nd), 'config.txt')
    with open(config_path, 'r') as f1:
        l1, l2 = f1.read().splitlines()[-2:]
        record = l1.split(' ')[-1] + ',' + l2.split(' ')[-1]
        b1_b2_records.append(record)



for beta1 in config['beta1']:
    for beta2 in config['beta2']:
        if str(beta1)+','+str(beta2) not in b1_b2_records:
            ...
            if not numbered_dirs:
                ...
                new_dir = str(max(numbered_dirs)+1)
